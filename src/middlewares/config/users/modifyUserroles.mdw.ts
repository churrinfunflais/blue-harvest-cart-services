...
import { updateIdepUser } from '../../../clients/idp.client.js';
...
const updateUser = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        if (!req.params.userId) throw new iError(MISSING_SCHEMA);

        const data = req.body as User;
        if (!req.objectSchema(data)) throw new iError(req.objectSchema.errors || []);

        const newUser = await updateIdepUser(data, req.workspace);
